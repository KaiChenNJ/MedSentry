...
import pandas as pd
import time
from structures.centralized import CentralizedStructure
...
def main():
    """
    Run the centralized structure system
    """
    parser = argparse.ArgumentParser(description="Run centralized structure system")
    ...
    for idx, row in dataset.iterrows():
        ...
        try:
            ...
            specialist_tokens = 0
            for specialist in system.specialists:
                specialist_tokens += sum(specialist.get_token_usage().values())

            leader_tokens = sum(system.leader.get_token_usage().values())
            evaluator_tokens = sum(evaluator.get_token_usage().values())
            case_tokens = specialist_tokens + leader_tokens + evaluator_tokens
